...
- Stable working prototype: **V2 build 30** on `codex/v2`
- Product type: browser-based deterministic painting-reference tool
- Current posture: stable deterministic V2 app with a good-enough module split; continue small feature/polish steps only after manual verification
- Latest checkpoint includes the workflow-stage sidebar, V2 processor modules, Color Study palette variants, Value Contours, curated outline presets, and small spacing/layout refinements.

## Current Working Product

Painter's Reference Lab V2 build 30 is a local-first reference-preparation tool for painters. It supports the path from loading an image through composition selection, drawing aids, painting studies, and exportable reference sheets.

The app is currently treated as a stable V2 prototype, not an active broad-refactor target. Workflow clarity, real-use testing, and small stability-first corrections matter more than architectural cleanup.
...
- Value Contours logic lives in `modules/observation-processors.js`
...
- V2 build 30 on `codex/v2` is the current stable prototype.
